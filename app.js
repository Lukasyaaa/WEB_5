document.addEventListener("DOMContentLoaded", () => {
    ...
    const isCorrectMinutes = (min) => {
        const parsedMinutes = parseInt(min, 10);
        return /^\d+$/.test(min) && min.length === 2 && parsedMinutes >= today.getMinutes() && parsedMinutes < 60;
    }
    ...
    } while (unitsParts.length !== 2 || !isCorrectHours(unitsParts[0]) || !isCorrectMinutes(unitsParts[1]));
    const neededDay = new Date(date + "T" + units);
   
    const timeContainerElement = document.querySelector(".date__countdown");
    ...
    subscribeElement.addEventListener("click", () => {
        timeContainerElement.innerHTML = "Вітаю, ви Встигли !";
